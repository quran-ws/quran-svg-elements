...
is not "the tanween pair's second stroke" — in 60 of 60 arbitrated samples it
...
renames these to `tanween-part`, would convert ~900 letters into marks
mushaf-wide while every existing gate stays green. Do not adopt it.

---

## Track 1 — R7 vs MushafDatabase (60 flagged + 20 unflagged meem-iqlab words)
...
| population | n | QPC writes ۢ/ۭ | uthmani writes ۢ/ۭ | MushafDatabase word holds a meem/iqlab label | MushafDatabase holds successive-tanween instead |
|---|---|---|---|---|---|
| R7-flagged | 60 | 0/60 | 60/60 | **0/60** | 60/60 (30 successive kasratan, 19 dammatan, 11 fathatan) |
| unflagged meem-iqlab | 20 | 20/20 | 20/20 | **20/20** (7 damma iqlab, 6 fatha iqlab, 3 kasra iqlab, 4 small meem) | 0/20 |

So R7's core claim — the print draws NO small م at idgham/ikhfa tanween, only
...
| what the flagged "meem-iqlab" turns out to be | n / 60 |
|---|---|
| bbox-IDENTICAL (all 4 edges < 0.5u) to a reference **letter** piece | 58 |
| bbox-identical to the reference's `kaf-hamza` letter-part (the ك dagger of شَكٍّۢ) | 2 |
| identical or overlapping the reference's successive-tanween stroke | **0** |
...
letter-part) at exactly the position uthmani's meem-rescue key expected a small
م. And in **all 60 words the tanween pair is already complete on our side**
(exactly one tanween master welded from exactly 2 strokes) — so the flagged
element is a THIRD element: the letter, missing from the word's body. Examples:
وَفَسَادٌۭ p186 (bodies hold و + فسا; the د is the "meem-iqlab"), هُزُوًۭا p300
...
- R7's diagnosis ("the pair's second stroke or its welded outline") and the
  proposed correction `tanween-part` are WRONG for the sampled population.
...
- **Trial corrector branch (b) is dangerous**: all 60 sampled elements sit
  ...
  rename letters into welded tanween parts across ~900 words. Bench, pixel
  checks and audit_marks stay green through that (renames move no ink and
  meem-iqlab is never count-demanded) — the same "clean score, broken word"
...
- Caveat on shape statistics: a true low-form ۭ meem can be 9.6u wide
  (مَّكَانٍۭ p434, confirmed `kasra iqlab` by the reference), inside the
...
|---|---|---|---|---|---|
| R1 fatha-below | 9 | 9 | — | 0 | 0 |
| R2 kasra-above | 4 | 4 | — | 0 | 0 |
| R2 kasratan-above | 5 | 5 | — | 0 | 0 |
...
| R10 pause-vs-ink | 9 | 2 (both sides of the كَيْدَهُنَّۚ→إِنَّهُۥ pause steal p239; مُسْتَمِعُهُم p525 holding المصيطرون's ۜ) | 6 (deficits real: the sign is drawn in ink but held by no word as a pause — عَمِلُوا۟ۖ p504, بَصْۜطَةًۭ p159's ۜ, تَقُولُۖ p91, وَٱلسَّلْوَىٰۖ p171, خَلْفَهُمْۖ p42, لَهُۥۖ-type) | 0 | 1 (ٱلْقَوْمِ p112 surplus) |
| R11 damma-deficit | 9 | 1 (أَعْيُنٌۭ p175: the dammatan holds THREE welded outlines — it swallowed the يُ damma; visible) | 7 (deficits real in data; the كُلٌّۭ ×4 all show the same 3-outline dammatan) | 0 | 1 (يَـٰٓأَيُّهَا p544 fused curl) |

Notes and refutations argued from the ink:

- **R1's blanket `proposed: kasra` (all 40 flags) is wrong for most of its own
  population.** The sample splits into (a) لأيات-family open-kasratan strokes
  below the band (q01, q26, q57, q69, q73, and likely q56) — the right name is
  the welded kasratan PAIR, not a plain kasra, and the R2 corrector's guarded
  swap already owns that pattern; and (b) cross-line fatha steals
  (أَلَمْ p543 → the fatha sits on النجوى's line; وَتَتَّقُوا۟ p99 → the fatha
  of the next line's وَأَن) where the ink IS a fatha of another word and
  renaming it "kasra" would be a second wrong. Detection: adopt. Rename: no.
- **R2-kasra-above**: 4/4 are real strokes above the band top. Rename→fatha is
  right where the word's text owes a fatha (مِنَ p415, إِلَّا p431/p576) and
  wrong for فِى p599, whose text has no fatha — that stroke belongs to a
  neighbour, and the fix is ownership, not name. One exclusion.
- **R2-kasratan-above**: 5/5 visually confirmed exactly as described — two
  welded strokes riding above the word top, the word's true kasratan below
  named fatha+fatha (q64/q73 are the two halves of the same p216 word).
  `fatha+fatha` is the correct correction; corrector branch (a)'s guards
  (fatha budget match + below-band strokes present) held in every sample.
...
- **R10/R11 are word-level count claims; a crop of the anchor mark cannot
  ...
  the dammatan weld absorbs a third waw (the word's plain damma), leaving the
  damma count short with zero stray ink — أَعْيُنٌۭ p175 shows it plainly, and
...
| **R7 proposed fix / corrector branch (b)** (`tanween-part` rename) | **DO NOT ADOPT** — the flagged ink is the word's final LETTER in 60/60 arbitrated samples; the correct fix is restore-to-body, per word, with the reference as the witness |
| R1 detection | SAFE TO ADOPT |
| R1 proposed `kasra` rename | DO NOT ADOPT as a blanket — split the population: лأيات-family → the R2 pair-swap; R9-adjacent steals → ownership repair first |
| R2 kasratan-above + corrector branch (a) | SAFE TO ADOPT (guards verified in every sample) |
| R2 kasra-above | ADOPT WITH EXCLUSION — rename only where the word's text owes a fatha (excludes فِى p599-type; fix those as transfers) |
...
| R11 | SAFE TO ADOPT (detector); root cause refined: 3-outline dammatan welds |

## Residue — one-glance items for Abdullah

| image | question |
|---|---|
| `marktype_verify/blind/q94.png` | ٱلْقَوْمِ p112 (5:25:12): three dot-sized blobs below the word, one held as a surplus "pause". A split ۛ, a neighbour's letter dots, or stray ink? |
| `marktype_verify/blind/q38.png` + `q68.png` | وَٱلشَّهَـٰدَةِ p548: the ش cluster is held as three-dots(2 blobs)+two-dots(1 blob) = 3 units, but the word's budget is 5 (ش3+ة2) — where are the ة dots held? |
| `marktype_verify/blind/q23.png` + `q29.png` | بِثَلَـٰثَةِ p66: same question — held dot units sum to 7 against a budget of 9 |
| `marktype_verify/blind/q63.png` | تَغِيضُ p250 (control, unflagged): its fatha sits ~10u above the band top, higher than every neighbour's — legitimate short-word high fatha, or a stolen stroke the "fatha-above is always legit" assumption hides? |
| `marktype_verify/blind/q85.png` | يَـٰٓأَيُّهَا p544 (R11): a curl welded beside the fatha over أَيُّ — is that the missing damma fused into the fatha outline? |
