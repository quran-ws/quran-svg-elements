# The two frozen families: hamzat_al_wasl (27) and small_waw (26)

## Plain-language summary

Both families were one systematic cause each, plus two loose ends.

**HamzatAlWasl.** The hamzat_al_wasl sign is drawn as a tiny ص above its word's first alef. In
...
word's hamzat_al_wasl lands on the FIRST word. That is 26 of the 27 flags: thirteen
pairs, each flagged twice (one word holding two hamzat_al_wasls, its neighbour none).
...
traced on p273, at oracle time ٱلْعَزِيزُ held only the stolen hamzat_al_wasl — its own
...
2. **Unowned adoption** (small_waw): a ۥ held by an UNOWNED atom is claimed by
...
| hamzat_al_wasl flags | 27 → **1** |
| small_waw flags | 26 → **1** |
| pages worse (RESEAT on vs off, current tree) | **0** |
| controls p1/p3 | byte-identical with the pass on vs off |

Attribution notes:
- The sweep-vs-sweep diff also shows ±3 fathah/tanwin_al_kasr drift on six pages
  (p111/p129/p155/p433/p518/p529, all iqlab-tanwin-al-kasr words). Verified NOT this
...
- p384 trades 2 mark flags for +1 interval flag: ٱلْعَلِيمُ now correctly holds
  its hamzat_al_wasl, which is DRAWN tucked inside ٱلْعَزِيزُ's kerned territory — the
...
Crops in `docs/defects/hamzat_al_wasl_smallwaw_samples/`, proposal items ready to append
in `docs/defects/hamzat_al_wasl_smallwaw_proposals.json` (not appended to proposals.json).

1. **p337 22:40:31 يَنصُرُهُۥٓ (small_waw 2/1).** The "second small_waw" is the
...
2. **p223 11:13:3 ٱفْتَرَىٰهُ (hamzat_al_wasl 1/0, also omitted_alif 1/0).** The ink is
   RIGHT — hamzat_al_wasl + dagger alef, exactly the 1441H print. quran.com's
   `text_uthmani` for this one word carries the rasm_imlai spelling (افْتَرَاهُ),
   so the budget demands zero of each. An upstream word-source data defect
   (their rasm_uthmani equals their rasm_imlai for word id 15424); needs a decision on
   ...
   range where rasm_uthmani==rasm_imlai and QPC disagrees, the same shape as the
   waqf-range rule.
