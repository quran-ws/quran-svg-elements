...
ٱجْتَمَعُوا۟ (22:73:17) held لَهُۥ's whole body (13.9–23.8), its ۖ pause, its
damma AND its fatha; لَهُۥ (22:73:18) held only وَإِن's و (321.4–331.4) plus
وَإِن's fatha; مِنْهُ (22:73:25) held لَهُۥ's small-waw ۥ (6.7–12.2, drawn in
line 2's band at the page's far left).

**Done:** 7 overrides on p341 — body/pause/damma/fatha → 22:73:18; و body +
fatha → 22:73:19; small-waw → 22:73:18. Each piece one step back, exactly the
approved chain (the fatha over ل had to travel with the body or both words'
slash budgets stay broken; measured below).

**Result:** page flags 5 → 1; the survivor (22:76:4 hamza 0/1) is unrelated and
pre-existing. لَهُۥ now holds body+fatha+damma+pause+small-waw — its complete
...
the دّ renamed fatha→kasra (text gives دِّ a kasra; the stolen body had dragged
the word's baseline down and the positional namer read it as a fatha).

**Result:** p59 fully clean — 1 flagged word → **0**. Emitted SVG verified:
مَا = 1 body + 1 fatha; يُؤَدِّهِۦٓ = 3 bodies (its allowed count).
...
line 12's start) + its shadda + its fatha, as approved — and لا meanwhile held
two pieces of يَحْمُومٍۢ from line 11's end: its final م (23.3–34.3) and the
second stroke of its kasratan pair (28.5–33.4; idgham tanween is drawn as a
stroke PAIR in this print, reported.json item 23).

**Done:** 5 overrides — body+shadda+fatha → 56:44:1; the م and the twin stroke
→ 56:43:3. Plus 2 `marks.json` entries naming both unpaired idgham twins
`kasratan` (28.5… in يحموم, 286.8–291.1 in بارد) — truthful family names; the
pair-welding itself is the known open kasratan-pairing family
(shape_notes), not part of this batch.

**Result:** flags 5 → 4, intervals 10 → **5**. لَّا is complete and clean;
بارد holds exactly its 3 pieces, and its real ب fatha (307.8–316.2, previously
misnamed kasra) renamed itself once the foreign fatha left. The two remaining
kasratan 2/1 flags (يحموم, بارد) are the unpaired twins counting as two — same
...
iqlab meem and fathatan pair. Mark audit 0 flags, intervals 0 (baseline
...
meem-iqlab MARK (it is the second د) and had 1 body for 2 allowed pieces; بيض
...
`QSVG_IQLATE` self-rescue names 301.4–304.7 `meem-iqlab` from the word's own
tanween anchor — the two-signals rule doing the naming rather than a forced
label.

**Result:** جدد = جد + د bodies + meem-iqlab mark + damma/fatha/dammatan/dot;
...
وَكَفَىٰ (33:3:4) holds fatha 5/3 in the baseline too, and its two surplus
fathas are drawn at line 5's far left over ٱللَّهِ (8.0–13.6 and 14.4–22.2,
