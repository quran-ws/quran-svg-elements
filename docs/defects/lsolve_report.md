...
1. **POSITION OWNS.** A violation is a slash/damma-family mark with zero
...
2. **THE TEXT CONSTRAINS.** Per-word, per-family |held − want| over
   fatha/kasra/fathatan/kasratan/damma/dammatan, counted after naming.
   **Hard receiver-room**: an arriving mark may fill a deficit, never create a
   surplus (+50 penalty = reject). Without it, p535 line 1 parked a kasratan
   on يُنزِفُونَ, which spells no tanween, because the global ledger still
   improved.
3. **NAMES FOLLOW POSITION.** A touched word's slash names are re-derived
   ...
   newly arrived, weld into one tanween named by the pair's side — **only if
   the word's text still owes that tanween** (two signals). The iqlab
   convention is applied TEXT-driven: a word whose text carries ۢ/ۭ that is
   short its tanween and long its same-side base has its leftmost base stroke
   renamed to the tanween (reported.json items 18/23; never geometric, per
...
4. **RESOLVE GLOBALLY.** Triggered lines expand to the line-set (±1 line's
   ...
   that overlap another word within 3u (the p350 fatha-at-x50.7 kind — never
...
**Scope.** Slash + damma families only. small-waw/small-ya are never
...
|---|---|---|
| 1. p350 lines 6–7 | لَا 24:3:9's true fatha (x327.9, over its own body) returns from يَنكِحُهَآ; the مشركة/والزانية tail re-owned; x14.9 (P1) untouched | **PASS.** fatha 327.9→لَا applied. Tail rotation applied: kasra 47.5→وَٱلزَّانِيَةُ (renamed fatha, its وَ), fatha 62.4→مُشْرِكَةًۭ (its كَ), kasra 66.9→مُشْرِكَةًۭ (its رِ), fathatan 101.6→زَانِيَةً (welded with its sitting pair-stroke at 98.4, counts as one ةً), مشركة's x50.7 stroke stays and is promoted to its iqlab fathatan (text ًۭ). x14.9 left exactly where it stands; excluded from ledgers as P1-pending. Bonus: fatha 304.9→يَرْمُونَ closes the pre-existing يرمون 1/2 flag (matches the interval audit's MARK-STEAL evidence). No rtl-order flags. |
| 2. p599 98:6 chain | أولئك's fatha back from شر, شر's back from البرية | **PASS.** fatha 187.4 (البرية)→شَرُّ, fatha 221.2 (شر)→أُو۟لَـٰٓئِكَ; أولئك 2/2, شر 1/1. The audit flag moves to the chain's end (البرية 1/2 — its second fatha is genuinely absent from the ledger; deficit, non-silent). NEIGHBOUR-BAD 2→0. A second 3-cycle on lines 1–2 also resolved (الذين→ءامنوا→نار→الذين) — flagged below for Abdullah. |
| 3a. p371 26:97 strays | the two kasras held by لَفِى resolved | **PASS, page fully clean** (2 flags → 0; intervals 2 → 0). kasra 305.6→ضَلَـٰلٍۢ, welded with ضلال's own mis-named stroke at 300.5 into its ٍۢ kasratan; kasra 324.0→ضلال renamed fatha (its لَ). لفى 1/1. |
| 3b. p267 | 16:2:10/11 stray | **Correctly refused, proposal emitted.** أَنْ cannot take its fatha back while it holds the y427 line-12 stroke, and no line-12 word has room for that stroke (السماوات family-full) — the same chain the xband pass refused. V unchanged → kept, recorded for eyes. No change to the page. |
| 3c. p535 56:33 stray pair | وَلَا's two stray fathas resolved | **PASS.** The pair (x8.3/x11.3, drawn at line 7's end, 3u apart) moves to مَقْطُوعَةٍۢ and welds into its ٍۢ kasratan (y246–252, where every kasratan on that line sits). ولا 4/2 fixed. The suspicious sitting element at (20,233) is re-derived by position to fathatan → the flag moves onto it (fathatan 1/0) — Abdullah's "(20,233) is suspicious" note made visible. Page 4 flags → 4, intervals 5 → 3. |
...
- **p350 marks +3 — the visibility proof** (the one per-audit worsening,
  ...
  - `لَا 2/1`: holds its returned true fatha (x327.9–334.8, y226.5–230.1,
  ...
  - `يَنكِحُهَآ 1/2` and `ٱلْمُحْصَنَـٰتِ 1/2`: donors of returned strays;
    each is now short the fatha its spelling owes because that ink is
  ...
  - `وَٱلزَّانِيَةُ 2/3`: its third fatha is either the P1 stroke or ink not
    in the ledger — P1's own open question, now visible.
- **Proposals**: 31 kept-component records over the 38 pages, none moved —
  24 unresolved chains for eyes, 7 classified trailing-tanween-after-hamza
...
1. **p599 lines 1–2 rotation** (applied): الذين's stray → ءَامَنُوا۟'s وَ;
   the stroke at x42–49/y38–41 → نَارِ as its رِ kasra; نار's held kasra at
   x65.5–71.8/y37–40 → ٱلَّذِينَ **renamed fatha** at y38.8, above الذين's
   letters. The first two look right; the third is the ledger-optimal close
   of the cycle but the stroke sits high — is it الذين's لَّ fatha, or فِى's
   kasra (فى is budget-blocked by the x75.8 stray it still holds — that
   residual chain is in the proposals)?
2. **p535 مَقْطُوعَةٍۢ fathatan 1/0**: the (20,233) element your note called
   suspicious now carries the flag directly. If it is not a mark at all, it
   is a P1-type proposals-page candidate.
3. **p350 fathatan residue**: مشركة's iqlab ًۭ is now its x50.7 stroke
