...
|---|---|---|
| Root contract (`data-schema/-version/-profile/-edition/-page/-text-digest`, §5) | bare `<svg>` from the artwork | `rewrite()` page assembly in `tools/assign_words.py`; digest computed over the composite text source (uthmani + KFGQPC waqf + DK budgets — see §4.1 below) |
...
| `data-role="letter-dot"` + `data-dot-count` (§6.2, §11.2) | dots emit as `data-kind="mark" data-mark="dot|two-dots|three-dots" data-mark-family="dots"` | emit() + `_MFAM` :2079; dot-count is mechanical from the current name. TEXT_WANT budgeting of dots is internal-record–based and unchanged through the bridge (v2 §6.2 itself licenses a combined processing bucket) |
...
| `data-placement` / `data-arrangement` axes (§12) | `data-form` = tanween-only `stacked|staggered` (emit:2117, ALLOWED_FORM :56). Seen above/below and meem high/low are NOT emitted today (see correction C2) | emit(); audit_taxonomy :134-139; seen sites keyed by `rare_places.json` rows (data, not code); meem placement from the `HARAKA` table :1548 (ۢ above ×510, ۭ below ×99) |
| `pause` removed as canonical name (§11.4) | `pause` is (a) the fallback when neither `waqf_types()` (sig) nor `waqf_places()` (place) resolves a subtype (emit:2100-2107) and (b) the TEXT_WANT budget key for the five waqf codepoints. imalah/ishmam/tashil/saktah/seen-reading are ALREADY split-out canonical names (audit_marks TEXT_WANT + RARE_SITES; audit_taxonomy ALLOWED_MARKS) — v2's claim they still hide in `pause` is stale (correction C4) | residue measurement in phase 0 (escalation E3); waqf family-level budgets stay per the 424/4,416 edition-disagreement rule — v2 §16.5 permits exactly this |
...
- Measure the `pause` residue: count emitted `data-mark="pause"` sites where
...
- Prove: logical mark counts == current master-path counts, family by family,
  mushaf-wide; relation cardinalities (muanaqah 2, sajdah line+sign, iqlab by
...
- New: `tools/sig_conflicts.py` — signatures occurring under conflicting
  textual/positional expectations (the small-noon/jeem-dot class); its output
...
- Edit: `tools/review_server.py`, `tools/build_variants_page.py`,
  ...
  values (`letter`, `letter-dot`, `header-ink`, …) never offered beside
...
- Edit `emit()` / `rewrite()` in `tools/assign_words.py`: root contract attrs;
  ...
  information — nothing to dual-emit except tanween, which also keeps
...
- Stop emitting `data-kind`, `data-mark`, `data-mark-part`, `data-fused`,
  `data-pair`, `data-iqlab`, `data-mark-family`, tanween `data-form`.
...
- **C1 — §11.2 small-noon.** "add the currently missing occurrence after
  visual verification": it is not missing. The 21:88 small-noon (p329) is
...
- **C2 — §17 migration map sources that do not exist.** The rows
  ...
  but tanween). Amend those rows to "nothing → `data-placement`", populated
  from `rare_places.json` rows (seen) and the `HARAKA` table (meem, ۢ×510
  above / ۭ×99 below).
- **C3 — §18 P0 sig length, confirmed and sharpened.** v2's catch is right and
  ...
  just documentation.
- **C4 — §11.4 scope of the `pause` removal.** "saktah / seen-reading /
  imalah / ishmam / tashil ... replace contextual `pause`" describes a state
  ...
  41:44). What `pause` still names is only (a) the unresolved-subtype waqf
...
3. **`waqf-mamnu` token → reserve, do not activate.** This print emits no لا
...
4. **Singleton category assignments → adopt v2's table as registry-only
   ...
   `meem-iqlab: dabt` etc. as written and move on.
...
E2 header eids adopted (role, no marks) · E3 pause blocks public until residue
zero · E4 one clean baseline re-pin after proven flag-identity · E5 the live
review campaign continues uninterrupted · E6 the letter-dot bridge seam is
...
- **E2 (v2 vs a one-day-old ruling): eids inside header groups.**
  ...
  eids too, after the p453 muanaqah dots hid un-referenceable inside a
  basmalah group.
  Options: (a) adopt v2 — eid + `data-role="header-ink"` per path, still no
...
- **E3 (ours is defensible on residue): when does `pause` actually die?**
  Question: emit() still falls back to `data-mark="pause"` when neither the
  ...
  does this); (b) let v2 public ship with `pause` as a documented legacy alias.
...
- **E5 (ours is defensible tonight): the live review campaign during phase 2.**
  ...
  them whenever it lands, and the UI flips between campaigns; (b) pause the
  ...
  tonight's known leak class (small-noon dot ↔ jeem-sign dot) is fenced even
  before the UI changes. Timing is Abdullah's.

- **E6 (v2 misses the counting seam, we can defend the bridge): letter-dot
