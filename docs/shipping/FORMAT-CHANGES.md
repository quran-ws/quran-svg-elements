...
boundaries come from. Breaking for consumers: the two key attributes are
renamed and a third is withdrawn.

| section | was | is | evidence |
|---|---|---|---|
| §6.1, §6.3, §9.2, groups table | `data-word-key` on word groups, `data-aid` on ayah groups and markers, `data-eid` on paths; `word_key` / `aid` / `eid` as JSON keys | **`data-word-key`**, **`data-ayah-key`**, **`data-element-id`**, and `word_key` / `ayah_key` / `element_id` in every index and sidecar. The abbreviations are gone; nothing about the values changed. | mechanical rename across the emitter, the audits, the bundle, the library and the demo. |
| §6.1, §10.9 | `data-w` on every word group, `w` as the second column of `index/words.json` and a field of `index/by-page/NNN.json` | **withdrawn.** A word is identified by `data-word-key="surah:ayah:word"` and nothing else — three ordinals, dense, in order, Hafs-specific. The release's cross-mushaf number is 1–77,434 over 77,432 written words (25685 is a word this mushaf does not write; 72:16:1 `وَأَلَّوِ` carries both 73950 and 73951), and those gaps belong to a scheme spanning every mushaf, not to this one. | `docs/HAFS-JSON-SOURCE.md`; `tools/build_word_by_word_translation_map.py` and `.cache/word_by_word_translation/word_key_to_w.json` deleted. |
| §6.1 (what makes the key sufficient) | — | the word-by-word-translation-translation release is now the pipeline's **only** source of word boundaries, so word N of an ayah here is word N of that ayah there. A consumer holding the release's data joins on position, with no id on either side. | `tools/build_word_by_word_translation_seg.py` derives the plan (4 splits, 1 fuse); `tools/audit_segmentation.py` proves count 0 / skeleton 0 over 6,236 ayahs, from the pipeline and from the emitted SVGs. |
| §6.1 | 15:7 `لَّوْ` `مَا` as two word groups | **one group**, `لَّوْمَا` — the release's boundary. Every later position in that ayah shifts down one. p262 stays pixel-identical; the ink is untouched. | `docs/MAQTU-MAWSUL.md`; bench case `lawma is one word`. |
...
| section | was | is | evidence |
|---|---|---|---|
| §9.2, §6.3, groups table | "12 `<g class="ayah-mark">` groups with no id — decorative rosettes"; 6,248 marker groups | The artwork draws each ornament of the opening spread **twice**, byte-identical and in place (7 pairs on p1, 5 on p2, no other page). The copy used to become its own marker and shift every id by one. Now it sits inside its ayah's group as `data-duplicate="1"`; 6,236 groups, all with `id` + `data-ayah-key`. Not removed: collapsing it darkens the anti-aliased rim by up to 57/255 (3,264 px on p1). | `tag_ayah_marks`; raster diff. |
...
| §6.5 | 4 paths on p17 without `data-kind` | `page_number` (2) and `running_head` (2): p17's page furniture, outside the viewBox. | `tag_page_furniture`. |
...
| §2, §6.1, §10.9 | all six text attributes on every word group in both profiles | **production carries `data-word-key`, `data-w` + `data-rasm-uthmani` only**; `rasm`, `rasm_imlai`, `search`, `qpc` ship in `index/by-page/NNN.json` and `words.json`, from the same cache and derivations; dev keeps them inline. Library: `createLoader({words: true})`, `page.attachWords()`. | emitter `_PROD` branch; `bundle_extract.text_forms`; `verify_bundle` cross-check. |
| §6.1 (new) | — | **`data-w`**, the global word id of the word-by-word-translation-translation source (KFGQPC UthmanicHafs v3.0 release), on every word group in both profiles and as `w` in `index/by-page/NNN.json` and `words.json` (new second column). Derived by `tools/build_word_by_word_translation_map.py`; page membership of the source agrees with ours on all 604 pages; segmentation on 6,235 of 6,236 ayahs (15:7's two pieces share one id). | `docs/HAFS-JSON-SOURCE.md`. |

Refused, with measurement, from the same report: sharing ayah_number digit
...
| **§7 An ayah is not a subtree** | The marker link (`id` / `data-mark` / `data-part` / `data-ayah-fragments`) is new, and this is the thing consumers get wrong. Worked queries for "select the whole ayah", "from an ayah find its medallion", "from a word find everything". |
...
| was | is | evidence |
|---|---|---|
| Status note: "the production profile removes `data-element-id`, `data-sig`, `data-mark-family` and the ligature wrapper" | Production removes the **ligature groups and the ayah polygons only**. `data-element-id`, `data-sig` and `data-mark-family` are in both. | `_PROD` is referenced at exactly three places in `assign_words.py` (2673 polygons, 2892 ligatures, 12394 polygon strip); byte-for-byte, `production` == `dev` minus those two things — proven by transforming the dev p3 into the prod p3 and comparing (`match: True`, 739,866 chars each). |
| `data-juz-start` / `data-hizb-start` / `data-nisf-start` / `data-rubu-al-hizb-start` listed under `<g class="division-mark">` with counts 73 / 141 / 168 / 671 | They are on **`<g class="ayah-fragment">`**, repeated on each fragment: 72 / 140 / 168 / 669 attribute instances, covering **30 / 60 / 60 / 240 distinct ayahs** — complete coverage of every division boundary. | corpus scan; the 199 `g.division-mark` groups carry only `data-mark`, `data-ayah-key`, `data-rubu-al-hizb`, `data-rubu-al-hizb-in-hizb`, `data-nisf`, `data-hizb`, `data-juz`. |
| §9.1 "the ayah-mark's `data-ayah-key` is REVERSED on 441 pages — do not use it" | **Fixed.** Markers are bound by position. On p3, `mk-2-6` now sits at screen y 57.9 (top, where 2:6 is printed) and `mk-2-16` at 523.8. | marker translate y, all 11 medallions on p3, descending correctly. |
...
| "34 emitted names ... `waqf` is a legacy fallback" (no registry accounting) | **34 emitted**, registry declares **36**, **35 active** (`waqf_mamnu` inactive), `waqf` active-but-never-emitted. `audit_taxonomy.py` prints `35 mark names` — that is the registry's active set, not the emitted set. Both numbers now explained. | `.cache/schema/mark-taxonomy.v2.json`; `audit_taxonomy` output. |
| §8.4 "space census: `data-rasm-imlai` 4,933 — an rasm_imlai convention, not word boundaries" | Kept, **plus** the new `data-search` row: 367 spaced values, 348 the `يا ` vocative and 19 others including `سواء السبيل` (60:1:48) and `نذير مبين` (71:2:6) — one printed word rendered as two modern words. | corpus scan. |
| §7.5 "a `data-sig` is not an identity" (two anecdotes) | Now quantified: **98 of 2,152 signatures serve more than one mark name**; `d2506e4f8b4e28e5` is `dot` 63,233 ×, `two_dots` 498 ×, `three_dots` 38 ×, `waqf_al_muanaqah` 3 ×; 85 signatures serve both `fathah` and `kasrah`. | corpus scan. |
| §7.2 fused-hamzah sites given as `21:28:4` p324, `22:76:4` p341 with no text | Confirmed; both are `أَيْدِيهِمْ`, and the `data-text="ا"` ligature holds exactly two paths (hamzah + fathah) and **no** body. | direct read of p324/p341. |

## Claims REMOVED (could not be verified)

- **"926 words carry a tanwin pair with no meem at ikhfa/idgham positions."**
  Not reproducible from the emitted files with any defensible definition (a
  naive count of tanwin-without-meem words gives 8,547). Dropped rather than
  restated.
...
`g.ayah-fragment` 13,510 → **13,489** · ayahs with >1 fragment 4,458 → **4,455** ·
`data-kind=mark` 436,708 → **436,629** · `body` 161,819 → **161,778** ·
named marks 436,706 → **436,627** · `header_ink` 5,550 → **5,670**
(2,300 surah-name + 3,370 basmalah) · `data-element-id` 598,527 → **598,407** ·
`data-sig` 598,512 → **598,392** · `data-mark-family` 122,203 → **122,191** ·
`fathah` 123,074 → **123,054** · `kasrah` 46,089 → **46,069** ·
`dot` 63,619 → **63,611** · `hamzat_al_wasl` 13,495 → **13,483** ·
...
- **4 paths on p17 carry no `data-kind` at all** — page ornaments outside both
  `#content` and `#ayah_marks`, passed through from the artwork. p17 is the
...
- **The `*-start` attributes cover all 240 rubʿ boundaries**, including the 41
  with no drawn rosette — so divisions should be read from `g.ayah-fragment`, never from
  the rosettes. Called out in §9.6.
- **`data-form` is absent on 48 of the 8,554 tanwin paths** (tanwin_al_fath 4,
  tanwin_al_damm 14, tanwin_al_kasr 30). The old text implied it was always present.
- **`small_noon` is the only *orthographic* sign with no family** — the old
...
All of §9's special cases: the 6 saktah/seen/imalah/ishmam/tashil/small_noon
...
meems; p350 `24:2:12` tanwin_al_damm drawn outside its word; p272 `16:48:10`
small_waw; p11 `2:70:1` sifr in the previous ligature; the unnamed mark p1
...
pages emitting one `g.ayah-fragment` per word (216 reopened groups); pages 1–2 with 8
