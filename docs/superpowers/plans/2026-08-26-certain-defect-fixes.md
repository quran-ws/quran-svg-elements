...
- Never move fathah/kasrah-family strokes between words (measured: +2 on four pages). Never use nearest-word for gap ink — direction beats distance.
...
### Task 1: Family A — iqlab-tanwin ligature surplus (12 words) — ✅ DONE 2026-08-26
...
own tanwin). 10 of 12 fixed; p203/p219 turned out to be pen-lift contour splits —
...
`أُمَّةٍۭ p85, حُجَّةٌۢ p104, أَلِيمٌۢ p136, شَدِيدٌۢ p143+p454, صَـٰلِحًۭا p203, تَارِكٌۢ p222, جِنَّةٌۢ p346+p429, بَارِدٍۢ p535, وَعَادٌۢ p566, نُنَجِّيكَ p219` — 11 of 12 end in tanwin + iqlab meem (`ٌۢ ٍۢ ًۭ`); the previews show a stray م drawn near the word counted as a letter piece.

**Files:** Modify: `tools/assign_words.py` (likely the iqlab/tanwin passes near `:4917` and `:5416`, or classify). Test: bench + `cmp_pages`.

**Root cause CONFIRMED** (see `docs/defects/iqlab_notation.md`, reported.json item 18): this print draws iqlab as ONE harakah + small م, never a tanwin pair. The `_IQTAN` pass at `assign_words.py:4917` knows this, but exits early (`continue  # already named`) whenever the tanwin is already named by the shape table — skipping the meem rescue, so the م stays counted as a letter piece.

- [ ] **Step 1 (verify per word):** For p143 `شَدِيدٌۢ`: `QSVG_TRACE=<x-of-the-م> python3 tools/assign_words.py hafs/kfqc 143`; confirm the surplus piece is the small م near the tanwin stroke and that `_IQTAN`'s "already named" branch is what skips it (add a temporary print or use `QSVG_ODBG`). Check the four `ۭ` words (p85, p203, p535, p566) specially — `:5416` claims the low form leaves no standalone glyph, yet they measure +1 piece.
- [ ] **Step 2 (fix):** In `_IQTAN` (`:4917`), decouple the meem rescue from the tanwin rename: run the rescue whenever the text has `ۢ`/`ۭ` and the word holds no `small_meem` mark, regardless of whether the tanwin is already named. Keep the existing size window (w 1.5–12u, h 3–12u) but measure whether d<14u reaches all 12 words before widening anything — widen only inside a measured empty band.
...
- **Vertical band steals: 55 marks ≥10u outside their line band** (his 12 confirmed +
  43 siblings). Sub-families: 11 waqf marks (all the "stole صلى from the word
  above/below" shape — 3 confirmed + 8 in the 10–14u zone: p45 p93 p108 p140 p149
  p214 p443 p525); ~30 fathahs; 4 small_meem; the rest singletons.
- **The real unit of repair is the LINE, not the word.** The width-swallow expansion
  ...
  line), p371 26:95–97 (11 words), p535 56:33–35 (6), p576 74:31 (5 band fathahs, juz
...
- **p254 ROOT CAUSE FOUND (Abdullah, reported.json item 20):** the print draws
  ...
  pieces — its "extra" fathahs/sukun are بعد's), and the word took a piece from
...
- **Multi-family mark hoarders: only 4 in the mushaf** — p254 أَهْوَآءَهُم (his "3
  fathahs, no dot" note), p223 افْتَرَاهُ, p239 إِنَّهُۥ, p341 ٱجْتَمَعُوا۟ (same line
  as the confirmed p341 small_waw steal → p341 is a line cluster too).
...
2. THE TEXT CONSTRAINS: each word's per-family counts equal its budget (waqf
   range, iqlab single-harakah+م, pen-lift welds included).
...
- [ ] **Step 3 (fix):** Repair the guard in the existing pass (position, not tag). Do NOT widen it: the fathah/kasrah slash families stay excluded from cross-word moves — where the stray mark IS a slash (p350, p418, p535 are fathahs), route the repair through the rtl/exchange evidence: both words sit on the same line in reading order, so the move is line-internal, not cross-line.
...
- [ ] **Step 6:** Bench case for p350 24:3:9 (fathah count = text budget). `reported.json`: update the p350 entry from "fixed" to re-fixed with cause. Commit.

---

### Task 3: Family C — mark drawn in another line's band, 16–41u (23 marks)

Sub-groups: waqf signs 37–41u (`p63, p64, p436`), small_meem 17–26u (`p129, p231, p415`), line-end fathahs 17–36u (`p71, p234, p267, p446, p507×3, p576×2, p577`), singletons (hamzah `p222, p260`; small_waw `p341`; maddah `p439`; kasrah `p567, p599`; dammah `p507`).
...
- [ ] **Step 5:** Bench case for one waqf (p63) and one fathah (p576). `reported.json`. Commit.
...
- [ ] **Step 1:** The ۩ sign is standalone (`:5740` treats hizb/rubu_al_hizb/division stars as no word's ink) — check whether the sajdah overline/sign here is counted as a word piece instead. Abdullah's `shape_notes.json` already says the sajdah line should group with the word below — read that note first.
