...
| | pinned baseline | now (2026-09-08, `.cache/sweeps/post-artwork`) |
|---|---|---|
| Flagged words (mark audit) | 559 | **7**, on two pages |
| Interval-audit flags | 172 | **3** |
| Pages clean (marks+intervals) | 276 | **600** of 604 |
| Pages clean incl. mark-TYPE audit | — | **not re-measured** — `audit_marktype.py 1 604` now reports 10,226 flags in categories (`unexplained` 4,049) that do not compare with the old 119; re-state this row before quoting it |
| Words emitted as two `<g class="word">` | 1158 | **0** |
| Open queue (`docs/defects/queue.json`) | — | **10** (6 auto, 4 judge), rebuilt from this sweep |
| Bench | SCORE 77, no failures, pixelfail 0 | **SCORE 136** over 33 cases, no failures, budget-mismatch 0, pixelfail 0 |
| `mushaf.js` suite | — | **321 pass, 0 fail** |

Every defect left in the mushaf is on **four pages** — p350, p399, p526, p555 —
and all of it is one shape: a word holds a run of ink its spelling does not
allow, a neighbour is short of one, and a mark follows the wrong body. Counting
cannot settle those; `tools/build_lastflags_page.py` draws each case with every
word of the ayah in its own colour for the eye to adjudicate
(`docs/defects/last_flags.html`).
