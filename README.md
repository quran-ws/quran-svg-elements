...
| **Search on `search`, not on the printed form** | The printed spelling writes some long vowels as marks; strip the marks and the vowel vanishes from the string, so a user's query returns nothing, quietly. Match against the `search` field, which is derived from the modern spelling. It is in the sidecar and in `index/words.json`, not on the page — the released bundle is the production profile and does not carry `data-search`. |
