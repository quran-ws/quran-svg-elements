...
and the position it takes in its run. Click once to mark it right, twice to mark it
wrong, a third time to clear it. Copy emits one line per judged shape with its id, so a
verdict can be applied to every word that draws it — the same way `labels.json` carries
one decision per mark signature across the whole mushaf.
"""
import argparse
import hashlib
import html
...
def card(c, scale=9):
    ...
    parts.append("</g></svg>")
    ex = c["example"]
    return ('<div class="sh" data-id="%s" data-ch="%s" data-form="%s" data-n="%d" '
            'data-page="%d" data-wid="%s" data-index="%d" onclick="mark(this)" '
...
def main():
    ...
    page = ("<!doctype html><meta charset=utf-8><title>Letter shapes — mark the good ones</title>"
            ...
            ".sh:hover{border-color:#999}.sh.good{border-color:#2a7;background:#f2fff8}"
            ...
            "<button onclick='copyAll()'>Copy verdicts</button><div id=tally>0 judged</div>"
            "<h1>Letter shapes</h1>"
            "<p>Every shape the split produced, one card per shape, with the number of words that "
            "draw it. <b>Click once for right, twice for wrong, three times to clear.</b> "
            ...
            "if(!s.contains('good')&&!s.contains('bad')){s.add('good')}"
            "else if(s.contains('good')){s.remove('good');s.add('bad')}"
            "else{s.remove('bad')}tally();}"
            "function tally(){const n=document.querySelectorAll('.sh.good,.sh.bad').length;"
            "const g=document.querySelectorAll('.sh.good').length;"
            "document.getElementById('tally').textContent=n+' judged ('+g+' right)';}"
            "function copyAll(){const out=[];document.querySelectorAll('.sh').forEach(e=>{"
            "const v=e.classList.contains('good')?'good':e.classList.contains('bad')?'bad':null;"
            "if(!v)return;out.push({id:e.dataset.id,letter:e.dataset.ch,form:e.dataset.form,"
