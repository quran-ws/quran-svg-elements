...
def main():
    ...
    page = ("<!doctype html><meta charset=utf-8><title>Letter shapes — mark the good ones</title>"
            ...
            "if(closed()){side=pts[0];if(preview())save();"
            "document.getElementById('ovmsg').textContent="
            "'the ink inside your loop is the letter \u2014 press invert if you meant the rest'}"
            "else document.getElementById('ovmsg').textContent="
            "'now click the part that IS the letter'}});"
            ...
            "function sideIsIn(){return true;}"
            "function sideIsInByClick(){if(!closed()||!side)return true;"
            "let c=false;for(let i=0,j=pts.length-1;i<pts.length;j=i++){"
            "const a=pts[i],b=pts[j];"
            "if(((a[1]>side[1])!==(b[1]>side[1]))&&"
            "(side[0]<(b[0]-a[0])*(side[1]-a[1])/(b[1]-a[1])+a[0]))c=!c}"
            "return c!==inv;}"
            ...
            "if(closed())rec.select=sideIsIn()?'in':'out';"
            ...
            "if(s.r===undefined?(s.t||s.v==='bad'):s.r)e.classList.add('rev');"
            "else e.classList.remove('rev');}"
            "function restore(){let o={};try{o=JSON.parse(localStorage.getItem(KEY)||'{}')}catch(err){}"
            "try{const legacy=JSON.parse(localStorage.getItem('letter_shape_verdicts')||'{}');"
            "Object.keys(legacy).forEach(k=>{if(!(k in o))o[k]=legacy[k]})}catch(err){}"
